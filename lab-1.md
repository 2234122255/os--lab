...
原因&解决办法：①函数签名不匹配：pthread_create期待的函数类型是```c
void*(*)(void*)
```,但我的add和sub函数的参数是spinlock_t*类型，需修改add、sub函数的参数为void*类型，并在函数内部进行强制类型转换；②编译时未链接到pthread库，应修改编译命令为：gcc test3-1.c -o tets3-1 -lpthread。  
